...
import ghMenu from "../ghMenu.mjs";
import { sleep } from "../../utils/general.mjs";
import ghRepos from "./ghRepos.mjs";
...
export const reportsMainMenu = async () => {
  ...
  const answers = await inquirer.prompt({
    ...
    choices: ["Get GH repos for a given username", "Return to previous menu", "Return to main menu", "Exit"],
  });
...
const handleMainSelection = async (selection) => {
  switch (selection) {
    case "Get GH repos for a given username":
      console.clear();
      await ghRepos();
